#!/usr/bin/env bash
# scripts/trend-now.sh
# Manually triggers the trending score update (Google Trends analysis) on the local FastAPI backend.
...
echo -e "${CYAN}🚀 Triggering trending score update and analysis...${RESET}"

RESPONSE=$(curl -s -X POST "${BACKEND_URL}/api/trending/trigger" \
